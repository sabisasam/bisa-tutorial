...
class FibonacciRpcClient(object):
    # If an instance of 'FibonacciRpcClient' gets created, this function will be executed.
    def __init__(self):
    ...
    # This function will be executed on every response message to the callback queue.
    def on_response(self, ch, method, props, body):
        # Check if the 'correlation_id' of the message matches the value from the request.
    ...
    # This function sends an RPC request and blocks until the answer is received.
    def call(self, n):
        self.response = None # Delete older response.
        self.corr_id = str(uuid.uuid4()) # Generate and save a unique 'correlation_id' number.
        ...
        self.channel.basic_publish(exchange='', # Default exchange, routes messages to queue specified
                                   ...
                                   properties=pika.BasicProperties( # Message properties.
                                        ...
                                        correlation_id = self.corr_id), # Useful to correlate RPC
                                                                        ...
                                                                        # discarded.)
                                   body=str(n)) # Message.
        # Wait until data sent to callback queue is accepted by function 'on_response'.
        while self.response is None:
            self.connection.process_data_events()
        # Return result, converted to an integer.
        return int(self.response)

# Create an instance of class 'FibonacciRpcClient' (directly when the client starts up).
