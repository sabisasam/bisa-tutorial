...
class Random1View(generic.ListView):
    ...
    def get_queryset(self):
        ...
        random_question = Question.objects.filter(
            pub_date__lte=timezone.now()).order_by('?').first()
...
class RandomView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if number_of_objects > 0:
            random_question = published_questions[
                randint(0, number_of_objects - 1)]
            return random_question
...
def questionsArchive(request, page_num):
    ...
    try:
        ...
        messages.error(
            request,
            'An error has occurred. The question could not be archived.')
